...
struct qcom_snps_hsphy {
	...
	/* Optional DPDM helper: exposes a "dpdm" regulator that places the
	 * PHY in UTMI non-driving mode so a PMIC charger can drive Dp/Dm for
	 * charger-type (APSD/QC) detection. */
...
static int qcom_snps_hsphy_dpdm_enable(struct regulator_dev *rdev)
{
	...
	reset_control_deassert(hsphy->phy_reset);

	/*
	 * For PMIC charger detection, place the PHY in UTMI non-driving mode
	 * so Dp/Dm are left high-Z and the charger can drive them itself.
	 */
...
static int qcom_snps_hsphy_dpdm_is_enabled(struct regulator_dev *rdev)
{
	struct qcom_snps_hsphy *hsphy = rdev_get_drvdata(rdev);

	return hsphy->dpdm_enabled;
}
...
static const struct regulator_ops qcom_snps_hsphy_dpdm_ops = {
	...
	.is_enabled	= qcom_snps_hsphy_dpdm_is_enabled,
};

static int qcom_snps_hsphy_regulator_init(struct qcom_snps_hsphy *hsphy)
{
	struct device *dev = hsphy->dev;
	...
	hsphy->dpdm_rdesc.owner = THIS_MODULE;
	hsphy->dpdm_rdesc.type = REGULATOR_VOLTAGE;
	hsphy->dpdm_rdesc.ops = &qcom_snps_hsphy_dpdm_ops;
	hsphy->dpdm_rdesc.name = kbasename(dev->of_node->full_name);

	cfg.dev = dev;
	cfg.init_data = init_data;
	cfg.driver_data = hsphy;
	cfg.of_node = dev->of_node;

	hsphy->dpdm_rdev = devm_regulator_register(dev, &hsphy->dpdm_rdesc,
						   &cfg);
	return PTR_ERR_OR_ZERO(hsphy->dpdm_rdev);
}
...
static int qcom_snps_hsphy_probe(struct platform_device *pdev)
{
	...
	ret = qcom_snps_hsphy_regulator_init(hsphy);
	if (ret)
		return dev_err_probe(dev, ret,
